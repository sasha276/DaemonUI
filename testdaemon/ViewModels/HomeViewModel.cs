...
public partial class HomeViewModel:ObservableObject
{
    #region Properties
        ...
        [ObservableProperty] string  _port      = "8000";
        ...
        [ObservableProperty] private HierarchicalTreeDataGridSource<CommandNode>? _commandsTreeSource;

        [ObservableProperty] private bool additionalWindow = false;
        
        public SettingsService SettingsService => SettingsService.Instance;
        
        public ThemeService ThemeService => ThemeService.Instance;
        
        partial void OnConnectedChanged(bool oldValue, bool newValue)
        {
            if (newValue == true) ConnectAsync();
            else if (newValue == false) DisconnectAsync();
        }
    ...
    #region Commands
        ...
        #region Device
            ...
            [RelayCommand]
            async Task OpenDeviceSessionAsync(DeviceEntry? device)
            {
                if (device == null) return;
                SelectedDevice = device;
                await SessionCreateAsync();
            }

            [RelayCommand]
            async Task DeviceInfoForAsync(DeviceEntry? device)
            {
                if (device == null) return;
                SelectedDevice = device;
                await DeviceInfoAsync();
            }
            ...
            [RelayCommand]
            async Task GetVersionDeviceAsync()
            {
                if(!CheckDevice()) return;

                try
                {
                    var v = await _client!.GetVersionDeviceAsync((ushort)SelectedDevice.Index);
                    Log($"GetVersion → \"{v}\"", LogLevel.Ok);
                }
                catch (Exception ex) { Log(ex.Message, LogLevel.Err); }
            }
            ...
            [RelayCommand]
            async Task CheckDeviceUartASync()
            {
                if(!CheckDevice()) return;
            
                try
                {
                    var has = await _client!.CheckUartDeviceAsync((ushort)SelectedDevice.Index);
                    Log($"CheckUart → {(has ? "present" : "not present")}", LogLevel.Ok);
                }
                catch (Exception ex) { Log(ex.Message, LogLevel.Err); }
            }
        ...
        #region Stream
            ...
            async Task StreamOpenAsync()
            {
                ...
                try
                {
                    ...
                    if (SettingsService.Instance.IsEnabled(SettingsService.Names.AutoShowWindow))
                    {
                        stream.FloatingWindow = true;
                    }
                    else if (SettingsService.Instance.IsEnabled(SettingsService.Names.AutoShowPanel))
                    {
                        AdditionalWindow = true;
                    }
            ...
            [RelayCommand]
            public async Task OpenStreamWindowAsync()
            {
                if (SelectedStream==null)
                {
                    Log("Выберите стрим",LogLevel.Warn);
                    return;
                }
        ...
        #region Other
            ...
            [RelayCommand]
            async Task SendPresetCommandAsync(CommandNode? cmd)
            {
                if (cmd == null || cmd.IsGroup) return;
                if (!CheckSession()) return;
                try
                {
            ...
            [RelayCommand]
            void GoToSettings() => NavigationService.Instance.GoToSettings();
        #endregion
    ...
    public HomeViewModel()
    {
        try
        {
            ...
            Log($"Commands loaded: groups={Commands.Groups.Count}, " +
                $"treeSource={(CommandsTreeSource == null ? "NULL" : "OK")}", LogLevel.Info);
        }
        catch (Exception ex) { Log($"LoadCommands: {ex.Message}", LogLevel.Err); }
    }
    private void LoadCommands()
    {
        CommandsTreeSource = TreeDataGridCommands.CreateTreeDataGridSource(
            Commands.Groups,
            node => _ = SendPresetCommandAsync(node));
    }
    ...
    private bool CheckDevice()
    {
        ...
        if(SelectedDevice==null) {Log("Выберите устройство",LogLevel.Warn);
            return false;
        }
